...
### Bug A
> 「他人のプロジェクトのタスク詳細URLを直接打つと、編集画面が出てしまい、保存もできてしまう。権限制御が効いていない」

**再現手順**
- alice でログイン
- alice が member でない project_id=3 のタスク（例: id=17「脆弱性スキャン定例実行」）の編集 URL `/tasks/17/edit` に直アクセス
- 編集できてしまうことを確認

### Bug B
> 「ダッシュボードがタスク数が増えると重い。SQL発行回数が異常」

**再現手順**
- DebugKit のツールバーから SQL Log を開く
- ダッシュボードを表示
- タスク件数 + 数件のクエリが発行されていることを確認

### Bug C
> 「期限が今日のタスクが『期限切れ』と表示されてしまう。仕様では『今日中＝期限内』のはず」

**再現手順**
- シードユーザのタスクで due_date = 今日 のものを探す
- ダッシュボードまたはタスク詳細で「期限切れ」表示になっていることを確認

### Bug D
> 「タスクにタグを付ける際、既存タグと同じ名前を入力すると tags テーブルに重複行ができる」

**再現手順**
- 既存タグ `urgent` がシードデータに存在することを確認
- タスク作成時に `Urgent`（先頭大文字、ケース違い）を入力して登録
- DB の `tags` テーブルを直接確認、または Tags 一覧で `urgent` と `Urgent` の2行が存在することを確認
...
### 推奨アプローチ
1. **先にテスト追加**：リファクタ前に挙動を固定するテストを書く
2. **Plan モード**で分離方針を ClaudeCode に提案させる
3. レビュー → 承認 → 実装
4. テスト緑を確認しながら段階的にコミット

### 分離先のヒント（必須ではない）
- `src/Service/TaskService.php`：ビジネスロジック
- `src/Model/Behavior/`：タグ正規化など Table 横断処理
- `EventListener`：通知メール、キャッシュ無効化

### 時間配分の目安
- 0〜10分：既存挙動を理解、テスト追加
- 10〜35分：分離・移植
- 35〜45分：テスト緑確認、コミット整理
